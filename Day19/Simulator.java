...
class Simulator {
  ...
  private boolean canWeBeatTheRecord(int minute, Situation situation) {
    ...
    int currentNumberOfGeodeRobots = situation.robots().getNumberOfRobots(ResourceType.GEODE);
    ...
    int currentAmountOfGeode = situation.resources().get(ResourceType.GEODE);
  ...
  private List<RobotTypeWithCost> defineRobotTypesToProduce(int minute, Situation situation) {
    ...
    if (minute == maxMinutes - 1) {
      return factory.getProducibleRobotTypes(situation.resources(), Set.of(ResourceType.GEODE));
    }
    ...
    if (minute == maxMinutes - 2) {
      return factory.getProducibleRobotTypes(
          situation.resources(), Set.of(ResourceType.GEODE, ResourceType.ORE, ResourceType.OBSIDIAN));
    }
  ...
  private void simulateRoundProducing(
    ...
    if (minute == maxMinutes) {
      int numberOfGeodes = newSituation.resources().get(ResourceType.GEODE);
